...
export function campaignSetup(route,signal){
  ...
  const values={};
  const customInputs={};
  const choice=(key,title,options,initial,customMax=0)=>{
    const group=el('fieldset',{className:'campaign-ai-modes'},el('legend',{text:title}));
    const checks=[];values[key]=initial;
    const custom=customMax?el('input',{type:'text',maxlength:customMax,'aria-label':`${title} 기타 입력`,placeholder:'원하는 내용을 직접 입력해주세요.',hidden:'',disabled:''}):null;
    if(custom)customInputs[key]={input:custom,title};
    for(const [value,label] of [...options,...(custom?[['__custom__','기타 (직접 입력)']]:[])]){
      const input=el('input',{type:'checkbox',value});input.checked=value===initial;checks.push(input);
      input.addEventListener('change',()=>{values[key]=value;for(const c of checks)c.checked=c.value===value;
        if(custom){custom.hidden=value!=='__custom__';custom.disabled=custom.hidden;if(!custom.hidden)custom.focus();}
      },{signal});
  ...
  choice('objective','목표',[['재구매 유도','재구매 유도'],['첫 구매 유도','첫 구매 유도'],['방문 유도','방문 유도']],'재구매 유도',500);
  ...
  choice('brand_tone','공통 말투',['다정하고 편안하게','간결하고 명확하게','차분하고 전문적으로'].map(v=>[v,v]),'다정하고 편안하게',200);
  ...
  choice('a_focus','A안 강조점',focuses,'혜택 강조',200);choice('b_focus','B안 강조점',focuses,'관계 강조',200);
  ...
  return {root,read(){
    ...
    for(const [key,{input,title}] of Object.entries(customInputs))if(values[key]==='__custom__'){
      resolved[key]=input.value.trim();
      if(!resolved[key])throw new Error(`${title}의 기타 내용을 입력해주세요.`);
    }
    if(resolved.a_focus===resolved.b_focus)throw new Error('A안과 B안의 강조점을 다르게 선택해주세요.');
    return {...resolved,benefit:benefit.value.trim(),target_value:target.value};
  }};
}
