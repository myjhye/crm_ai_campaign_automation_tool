...
const item=(label,prompt)=>({label:prompt,prompt});
export function followups(message,hint){
  ...
  if(response.result_type==='campaign_comparison'){
    ...
    const subject=hint?.kind==='campaign_list'?`위 캠페인 ${data.campaigns.length}개`: '전체 캠페인';
    return [
      item('클릭률로 다시 비교',`${subject}을 클릭률 순으로 다시 비교해줘. 전환율 순위와도 비교해볼게`),
      item('기여 매출로 다시 비교',`${subject} 중 기여 매출이 가장 큰 3개를 보여줘`),
  ...
  if(keys.includes('new_customers'))return [item('', '선택 기간에 가입했고 아직 구매하지 않은 고객을 찾아줘'),item('', '신규 고객 수와 함께 볼 구매 전환율도 알려줘')];
  if(keys.includes('dormant_customers'))return [item('', '휴면 고객 중 60일 이상 미구매하고 누적 구매액이 30만원 이상인 고객을 찾아줘')];
  if(keys.includes('active_customers'))return [item('', '활동한 고객이 실제 구매로 이어졌는지 구매 전환율을 알려줘'),item('', '장바구니에 담았지만 구매 이벤트가 없는 고객을 찾아줘')];
  if(keys.includes('repeat_purchase_rate'))return [item('', '완료 주문이 3건 이상인 반복 구매자는 몇 명인지 찾아줘'),item('', '반복 구매자 중 이메일에 동의한 고객을 세그먼트 저장 초안으로 만들어줘')];
  if(keys.includes('purchase_conversion_rate'))return [item('', '구매로 이어지지 않은 장바구니 고객을 찾아줘'),item('', '신규 고객 중 구매 이력이 없는 고객을 찾아줘')];
  return [
