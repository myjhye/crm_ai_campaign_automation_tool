...
import {recentTurns} from './history.js';
...
export function renderAI(root,signal){
  ...
  const state=memory;let busy=false,pendingScroll=0;
  ...
  const jump=button('↓ 새 답변 읽기',()=>{jump.hidden=true;readLatest();},signal,'ai-new-answer');
  jump.hidden=true;
  function readLatest(){
    const latest=results.querySelector('.ai-message-assistant:last-of-type');
    if(latest)results.scrollTop+=latest.getBoundingClientRect().top-results.getBoundingClientRect().top-16;
  }
  results.addEventListener('scroll',()=>{state.scrollTop=results.scrollTop;},{signal});
  ...
  panel.append(header,results,jump,composer.node);root.replaceChildren(panel);
  ...
  function draw(scroll=false){
    ...
    const readingHistory=scroll==='answer'&&Math.abs(previousTop-pendingScroll)>8;
    ...
    if(!started){results.scrollTop=0;jump.hidden=true;}
    else if(scroll===true){jump.hidden=true;results.scrollTop=results.scrollHeight;pendingScroll=results.scrollTop;}
    else if(scroll==='answer'&&!readingHistory){jump.hidden=true;readLatest();}
    else {results.scrollTop=previousTop;if(readingHistory)jump.hidden=false;}
  }
  const restoredScroll=state.scrollTop||0;
  composer.fill(state.input);draw();results.scrollTop=restoredScroll;
  ...
  async function submit(prompt){
    ...
    const recent_turns=recentTurns(state.messages);
    ...
    try{
      ...
      const response=await request('/ai/chat',{method:'POST',signal,body:{prompt,recent_turns,dataset_id:route.dataset,...period,reference_at:period.to,conversation_id:state.id,...(state.hint?{context_hint:state.hint}:{})},
        validate:r=>['metric','segment_preview','clarification','campaign_comparison','campaign_draft','copy_recommendation'].includes(r?.result_type)&&typeof r.message==='string'&&r.data&&isUUID(r.dataset_id)});
    ...
    finally{if(!signal.aborted){busy=false;composer.busy(false);draw('answer');composer.input.focus({preventScroll:true});}}
  }
