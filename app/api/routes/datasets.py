...
def create_dataset(payload: DatasetCreate, request: Request, session: DB):
    return repository.describe(session, [service.create_dataset(session, payload, UUID(request.state.request_id))])[0]
...
def list_datasets(pagination: Annotated[Pagination, Query()], session: DB):
    ...
    return Page[DatasetResponse](items=repository.describe(session, rows), total=total, page=pagination.page, page_size=pagination.page_size)


@router.get("/{dataset_id}", response_model=DatasetResponse)
def get_dataset(dataset_id: UUID, session: DB):
    return repository.describe(session, [service.require_dataset(session, dataset_id)])[0]


@router.put("/{dataset_id}", response_model=DatasetResponse)
def update_dataset(dataset_id: UUID, payload: DatasetUpdate, request: Request, session: DB):
    return repository.describe(session, [service.update_dataset(session, dataset_id, payload, UUID(request.state.request_id))])[0]
