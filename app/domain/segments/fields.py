...
def expressions(snapshot, dataset_id, reference):
    ...
    category = select(Product.category).select_from(Order).join(OrderItem, OrderItem.order_id == Order.id).join(Product, Product.id == OrderItem.product_id).where(Order.dataset_id == dataset_id, Order.customer_id == snapshot.c.id, Order.status == 'COMPLETED', Order.purchased_at < reference).group_by(Product.category).order_by(func.sum(OrderItem.amount).desc(), Product.category).limit(1).correlate(snapshot).scalar_subquery()
    return {'days_since_last_purchase': func.floor(func.extract('epoch', literal(reference) - snapshot.c.last_purchase_at) / 86400),
