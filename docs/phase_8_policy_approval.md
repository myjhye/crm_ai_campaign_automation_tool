...
`campaign_deliveries`는 정책 조회에 필요한 최소 원장으로 먼저 만들었고, 단계 9에서 run, A/B 배정과 실제 모의 발송 결과를 연결했다.
...
캠페인 목록의 삭제는 `archived_at`을 기록하는 보관 처리이며 목록·상세 API에서는 더 이상 노출하지 않는다. DRAFT·REVIEW·APPROVED·COMPLETED는 보관할 수 있고 모의 발송 중인 RUNNING만 차단한다. 승인·검수·발송 이력을 물리적으로 연쇄 삭제하지 않는다.
