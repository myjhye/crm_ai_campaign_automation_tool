...
OpenAI Responses API 도구 선택 경로, 모의 provider, 지표 조회·조건 미리보기·저장 제안, 확인 API와 우측 패널을 구현했다. 키 설정 후 로컬 `.env`를 `AI_MODE=live`로 변경했다. 실제 OpenAI 도구 선택, DB와 일치하는 활성 고객 지표, 실제 세그먼트 미리보기를 검증했다. 데모 generator v2의 300명 샘플에서는 같은 휴면 VIP 조건이 30명으로 재현된다. 실제 LLM과 브라우저를 함께 사용한 확인 저장 전체 시연은 별도 확인 대상이다.
