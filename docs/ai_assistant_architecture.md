...
| 저장 없는 문구 추천·수정 | recommend_copy | LLM이 작성한 A/B 문구·가설, 저장 버튼 없음 |
...
전송할 때는 질문과 대기 상태로 이동하지만, 답변이 도착하면 끝이 아니라 답변 시작 부분을 맞춘다. 응답을 기다리는 동안 사용자가 다른 위치로 스크롤했다면 그 위치를 유지하고 “새 답변 읽기” 버튼을 제공한다. 저장·문맥 변경으로 다시 그릴 때도 스크롤 위치를 유지한다. 같은 범위에서 화면을 다녀오면 읽던 위치를 복원한다. 긴 답변의 시작 정렬·이전 대화 읽기·버튼 이동을 포함한 AI 화면 E2E 5개가 통과했다.
...
`followups.js`가 마지막 구조화 결과·현재 문맥·저장 여부로 최대 세 개의 후속 질문을 만든다. 화면에는 짧은 한 줄을 표시하고, 클릭 시 대상 조건을 포함한 질문을 입력창에 채운다. 별도 LLM 호출은 없으며 임의 대화 전문을 해석하는 기능은 아니다.
...
| `frontend/src/features/ai/history.js` | 최근 6개 대화의 제한된 문맥 구성 |
...
| `app/ai/advice.py` | 저장 없는 추천의 A/B 구조·채널 길이·일반 텍스트 검증 |
...
현재는 recent_turns로 최근 최대 6개 대화, 항목당 최대 2,000자를 모델에 전달한다. 사용자 질문과 AI 안내, 조회 조건 설명, 추천 문안을 포함하되 고객 프로파일·행·집계 숫자 표를 자동 첨부하지 않는다. 각 항목은 safe_prompt 검사 대상이다. 클라이언트 대화는 비신뢰 참고 자료이며 권한·DB 사실·저장 승인 근거가 아니다. 서버에 대화 원문을 영속 저장하거나 로그로 남기지 않는다. 브라우저 모듈 메모리는 같은 범위에서 다른 화면을 다녀오면 복원하며, 새 대화·데이터셋·기간 변경·새로고침 때 초기화한다. AbortController는 화면 이탈 요청과 늦은 응답 반영을 막는다. 이미 시작된 서버 저장을 브라우저 취소만으로 롤백한다는 보장은 아니다.
...
“이 고객에게 보낼 A/B 문구 추천”은 세그먼트 저장 없이 recommend_copy로 처리한다. “추천만 해줘, 저장하지는 않을 거야”도 최근 대화의 고객 조건과 문구 요청으로 연결한다. 명확한 추천 전용 요청에는 추천·확인 질문 도구만 허용하고 서버도 잘못 선택된 조회·저장 도구를 실행하지 않는다. 채널이 없으면 UNSPECIFIED 일반 문구, 혜택이 없으면 혜택을 지어내지 않는 문구를 생성하도록 지시한다. A/B 구조·채널 길이·HTML/변수·기본 금지 표현을 검증하나 모든 문구의 사실성을 보장하지는 않는다. 추천은 AIActionProposal·Segment·Campaign을 생성하지 않는다. 실제 캠페인 저장 요청은 기존 확인 저장 경로를 사용한다.

새 고객 미리보기는 이전 저장 세그먼트와 다른 대상일 수 있으므로 기존 저장 리소스 문맥은 해제한다. 자연어 대화 문맥과 저장 가능한 리소스 참조를 구분한다.
