...
단계 6까지 DB·worker, 공개 데이터셋·감사 이력, CSV 적재·샘플 생성, 공통 화면·고객·대시보드, 세그먼트 조건 검증·미리보기·저장을 구현했다. AI-A의 provider·도구·패널·확인 저장을 추가했고 최신 마이그레이션은 `004a`다. 실제 API 키를 이용한 LLM 시연은 아직 미검증이다. 캠페인 업무는 이후 단계다. 패키지 설치로 생성되는 `*.egg-info/`나 `.venv/` 내부는 수정 대상이 아니다.
...
다음 확인은 **AI-A 실제 키 기반 시연**이며 다음 업무 구현은 **단계 7 캠페인 → AI-B 초안·카피 연결**이다. 발송·피로도처럼 후속 원천에 의존하는 항목은 해당 단계에서 연결한다.
