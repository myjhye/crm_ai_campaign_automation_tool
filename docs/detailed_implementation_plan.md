...
단계 9까지 DB·worker, 공개 데이터셋·감사 이력, CSV 적재·샘플 생성, 공통 화면·고객·대시보드, 세그먼트, 캠페인 편집, 정책 검수·승인과 모의 발송을 구현했다. AI-A~C로 지표·세그먼트, 캠페인 초안, 정책 검수 도구를 연결했으며 최신 마이그레이션은 `007`이다. 다음 업무 구현은 단계 10 성과·실험·보고서다. 패키지 설치로 생성되는 `*.egg-info/`나 `.venv/` 내부는 수정 대상이 아니다.
...
다음 업무 구현은 **단계 10 성과·실험·보고서**다. 단계 9는 승인 스냅샷 기반 실행, 멱등 요청, 안정적 A/B 배정, 모의 반응 이벤트와 주문 생성을 구현했다. 단계 10에서 이 원장을 공통 성과 API와 보고서로 집계한다.
