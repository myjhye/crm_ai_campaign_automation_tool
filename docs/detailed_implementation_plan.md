...
단계 8까지 DB·worker, 공개 데이터셋·감사 이력, CSV 적재·샘플 생성, 공통 화면·고객·대시보드, 세그먼트, 캠페인 편집, 정책 검수·승인을 구현했다. AI-A~C로 지표·세그먼트, 캠페인 초안, 정책 검수 도구를 연결했으며 최신 마이그레이션은 `006a`다. 다음 업무 구현은 단계 9 모의 발송이다. 패키지 설치로 생성되는 `*.egg-info/`나 `.venv/` 내부는 수정 대상이 아니다.
...
다음 업무 구현은 **단계 9 모의 발송**이다. AI-C는 단계 8과 같은 정책 서비스를 호출해 검수 스냅샷을 남기고 승인 화면으로 연결한다. 발송 이력 원장에 실제 모의 발송 결과를 쓰는 작업은 단계 9에서 진행한다.
