...
2026-09-22 추가 개선: [AI 워크스페이스 재설계](ai_assistant_architecture.md)를 구현했다. 결정론적 인사이트, 업무 예시, 화면 이동 후 대화 복원, 서버 검증 문맥 칩, 저장된 세그먼트에서 캠페인 제안·확인 저장, 완료 캠페인 성과 비교를 연결했다. 개별 캠페인의 검수와 성과 분석은 Campaigns·Experiments에 유지한다. Alembic `008`은 실행 로그의 conversation ID만 추가하며 대화 원문은 저장하지 않는다.
