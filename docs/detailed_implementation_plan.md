...
단계 6까지 DB·worker, 공개 데이터셋·감사 이력, CSV 적재·샘플 생성, 공통 화면·고객·대시보드, 세그먼트 조건 검증·미리보기·저장을 구현했다. 최신 마이그레이션은 `004`다. 다음은 AI-A의 자연어 지표 조회·세그먼트 연결이며 캠페인 업무는 이후 단계다. 패키지 설치로 생성되는 `*.egg-info/`나 `.venv/` 내부는 수정 대상이 아니다.
